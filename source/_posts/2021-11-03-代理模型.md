...
对于形式一，给定数据集为$\boldsymbol{X}=\left\{\boldsymbol{x}^{1}, \boldsymbol{x}^{2}, \ldots, \boldsymbol{x}^{n}\right\}^{T}$​，对应的目标函数为$\boldsymbol{y}=\left\{y^{1}, y^{2}, \ldots, y^{n}\right\}^{T}$​克里金法假设所有数据之间都服从n维的正态分布。所以目标函数$\boldsymbol{y}$​​是一个随机过程，里面的每一个变量 ![[公式]](https://www.zhihu.com/equation?tex=y_i) 都是一个随机变量。我们把这个随机过程记做：
$$
\left(\begin{array}{c}
